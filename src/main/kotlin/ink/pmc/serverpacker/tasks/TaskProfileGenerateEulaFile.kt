...
object TaskProfileGenerateEulaFile : Task {
    ...
    override fun run(profile: Profile) {
        ink.pmc.serverpacker.logger.info("Generating Eula file for ${profile.name}...")

        val file = File(profile.profileDir, "eula.txt").fileCheckOrCreate()
        val content = "eula=${profile.eula}"

        if (!profile.eula) {
            ink.pmc.serverpacker.logger.warn("You haven't agreed the Mojang Eula. The server won't start properly.")
        }
